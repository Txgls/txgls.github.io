<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Txgls Scripts - FiveM QBCore Collection</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap');
        
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }
        
        body {
            font-family: 'Roboto', sans-serif;
            background: linear-gradient(135deg, #1a1a1a 0%, #2d1810 100%);
            color: #fff;
            overflow-x: hidden;
        }
        
        .container {
            min-height: 100vh;
            position: relative;
        }
        
        /* Animated background elements */
        .bg-elements {
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            pointer-events: none;
            z-index: -1;
        }
        
        .bg-circle {
            position: absolute;
            border-radius: 50%;
            background: linear-gradient(45deg, rgba(139, 69, 19, 0.1), rgba(160, 82, 45, 0.1));
            animation: float 20s infinite ease-in-out;
        }
        
        .bg-circle:nth-child(1) {
            width: 200px;
            height: 200px;
            top: 20%;
            left: 80%;
            animation-delay: 0s;
        }
        
        .bg-circle:nth-child(2) {
            width: 150px;
            height: 150px;
            top: 60%;
            left: 10%;
            animation-delay: 7s;
        }
        
        .bg-circle:nth-child(3) {
            width: 100px;
            height: 100px;
            top: 30%;
            left: 15%;
            animation-delay: 14s;
        }
        
        @keyframes float {
            0%, 100% { transform: translateY(0px) rotate(0deg); }
            33% { transform: translateY(-30px) rotate(120deg); }
            66% { transform: translateY(15px) rotate(240deg); }
        }
        
        /* Header Section */
        .header {
            text-align: center;
            padding: 60px 20px;
            background: linear-gradient(45deg, rgba(139, 69, 19, 0.2), rgba(160, 82, 45, 0.2));
            backdrop-filter: blur(10px);
            border-bottom: 2px solid rgba(139, 69, 19, 0.3);
        }
        
        .logo {
            font-size: 3.5rem;
            font-weight: 700;
            color: #ff6b35;
            margin-bottom: 10px;
            text-shadow: 2px 2px 4px rgba(0,0,0,0.5);
            animation: glow 2s ease-in-out infinite alternate;
        }
        
        @keyframes glow {
            from { text-shadow: 2px 2px 4px rgba(0,0,0,0.5), 0 0 20px rgba(255, 107, 53, 0.3); }
            to { text-shadow: 2px 2px 4px rgba(0,0,0,0.5), 0 0 30px rgba(255, 107, 53, 0.6); }
        }
        
        .subtitle {
            font-size: 1.3rem;
            color: #d4af37;
            font-weight: 300;
            margin-bottom: 20px;
        }
        
        .description {
            font-size: 1.1rem;
            color: #ccc;
            max-width: 800px;
            margin: 0 auto;
            line-height: 1.6;
        }
        
        /* Scripts Section */
        .scripts {
            padding: 80px 20px;
            max-width: 1200px;
            margin: 0 auto;
        }
        
        .section-title {
            text-align: center;
            font-size: 2.5rem;
            color: #ff6b35;
            margin-bottom: 50px;
            font-weight: 500;
        }
        
        .scripts-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(500px, 1fr));
            gap: 40px;
            margin-bottom: 60px;
        }
        
        .script-card {
            background: linear-gradient(145deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.05));
            backdrop-filter: blur(10px);
            border: 1px solid rgba(139, 69, 19, 0.3);
            border-radius: 20px;
            padding: 40px;
            transition: all 0.4s ease;
            position: relative;
            overflow: hidden;
            cursor: pointer;
            text-decoration: none;
            color: inherit;
            display: block;
        }
        
        .script-card::before {
            content: '';
            position: absolute;
            top: -50%;
            left: -50%;
            width: 200%;
            height: 200%;
            background: linear-gradient(45deg, transparent, rgba(255, 107, 53, 0.1), transparent);
            transform: rotate(45deg);
            transition: all 0.6s ease;
            opacity: 0;
        }
        
        .script-card:hover::before {
            opacity: 1;
            animation: shine 0.6s ease-in-out;
        }
        
        .script-card:hover {
            transform: translateY(-15px) scale(1.02);
            border-color: rgba(255, 107, 53, 0.7);
            box-shadow: 0 25px 50px rgba(255, 107, 53, 0.3);
        }
        
        @keyframes shine {
            0% { transform: translateX(-100%) translateY(-100%) rotate(45deg); }
            100% { transform: translateX(100%) translateY(100%) rotate(45deg); }
        }
        
        .script-icon {
            font-size: 4rem;
            margin-bottom: 25px;
            display: block;
            text-align: center;
        }
        
        .script-title {
            font-size: 2rem;
            color: #ff6b35;
            margin-bottom: 15px;
            font-weight: 600;
            text-align: center;
        }
        
        .script-subtitle {
            font-size: 1.2rem;
            color: #d4af37;
            margin-bottom: 20px;
            text-align: center;
            font-weight: 300;
        }
        
        .script-desc {
            color: #ccc;
            line-height: 1.6;
            font-size: 1.1rem;
            text-align: center;
            margin-bottom: 30px;
        }
        
        .features-list {
            list-style: none;
            padding: 0;
        }
        
        .features-list li {
            color: #bbb;
            padding: 8px 0;
            border-bottom: 1px solid rgba(139, 69, 19, 0.2);
            display: flex;
            align-items: center;
        }
        
        .features-list li:last-child {
            border-bottom: none;
        }
        
        .features-list li::before {
            content: '✓';
            color: #d4af37;
            font-weight: bold;
            margin-right: 15px;
            font-size: 1.2rem;
        }
        
        .cta-button {
            background: linear-gradient(45deg, #ff6b35, #ff8c42);
            border: none;
            padding: 15px 30px;
            border-radius: 30px;
            color: white;
            font-weight: 600;
            font-size: 1.1rem;
            margin-top: 20px;
            width: 100%;
            transition: all 0.3s ease;
            text-transform: uppercase;
            letter-spacing: 1px;
        }
        
        .script-card:hover .cta-button {
            background: linear-gradient(45deg, #ff8c42, #ff6b35);
            transform: translateY(-2px);
            box-shadow: 0 10px 20px rgba(255, 107, 53, 0.4);
        }
        
        /* Stats Section */
        .stats {
            background: linear-gradient(45deg, rgba(139, 69, 19, 0.2), rgba(160, 82, 45, 0.2));
            padding: 60px 20px;
            text-align: center;
        }
        
        .stats-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
            gap: 40px;
            max-width: 800px;
            margin: 0 auto;
        }
        
        .stat-item {
            padding: 20px;
        }
        
        .stat-number {
            font-size: 3rem;
            color: #d4af37;
            font-weight: 700;
            display: block;
            margin-bottom: 10px;
        }
        
        .stat-label {
            color: #ccc;
            font-size: 1.1rem;
            text-transform: uppercase;
            letter-spacing: 1px;
        }
        
        /* Footer */
        .footer {
            background: #1a1a1a;
            padding: 40px 20px;
            text-align: center;
            border-top: 2px solid rgba(139, 69, 19, 0.3);
        }
        
        .footer-text {
            color: #888;
            font-size: 1rem;
        }
        
        .footer-highlight {
            color: #ff6b35;
            font-weight: 500;
        }
        
        /* Responsive Design */
        @media (max-width: 768px) {
            .logo {
                font-size: 2.5rem;
            }
            
            .scripts-grid {
                grid-template-columns: 1fr;
                gap: 30px;
            }
            
            .script-card {
                padding: 30px;
            }
            
            .section-title {
                font-size: 2rem;
            }
            
            .script-title {
                font-size: 1.7rem;
            }
        }
        
        /* Scroll animations */
        .fade-in {
            opacity: 0;
            transform: translateY(30px);
            transition: all 0.6s ease;
        }
        
        .fade-in.visible {
            opacity: 1;
            transform: translateY(0);
        }
        
        /* Pulse animation for call-to-action */
        @keyframes pulse {
            0% { box-shadow: 0 0 0 0 rgba(255, 107, 53, 0.7); }
            70% { box-shadow: 0 0 0 10px rgba(255, 107, 53, 0); }
            100% { box-shadow: 0 0 0 0 rgba(255, 107, 53, 0); }
        }
        
        .script-card:hover .cta-button {
            animation: pulse 2s infinite;
        }
    </style>
</head>
<body>
    <div class="container">
        <div class="bg-elements">
            <div class="bg-circle"></div>
            <div class="bg-circle"></div>
            <div class="bg-circle"></div>
        </div>
        
        <header class="header">
            <h1 class="logo">🔧 Txgls Scripts</h1>
            <p class="subtitle">Premium FiveM QBCore Script Collection</p>
            <p class="description">
                Professional, optimized, and feature-rich scripts designed specifically for FiveM QBCore servers. 
                Enhance your roleplay experience with our carefully crafted solutions.
            </p>
        </header>
        
        <section class="scripts">
            <h2 class="section-title fade-in">Available Scripts</h2>
            <div class="scripts-grid">
                <a href="#butcher" class="script-card fade-in" onclick="showButcherDetails()">
                    <span class="script-icon">🥩</span>
                    <h3 class="script-title">ButcherJob</h3>
                    <p class="script-subtitle">Professional Meat Processing</p>
                    <p class="script-desc">
                        Immersive butcher job experience with realistic meat processing mechanics and engaging roleplay opportunities.
                    </p>
                    <ul class="features-list">
                        <li>Realistic Processing System</li>
                        <li>Fully Configurable Settings</li>
                        <li>Roleplay Enhanced Features</li>
                        <li>QBCore Optimized Performance</li>
                    </ul>
                    <button class="cta-button">View Details</button>
                </a>
                
                <a href="#loadscreen" class="script-card fade-in" onclick="showLoadscreenDetails()">
                    <span class="script-icon">🎮</span>
                    <h3 class="script-title">LoadScreen</h3>
                    <p class="script-subtitle">Custom Server Loading Experience</p>
                    <p class="script-desc">
                        Beautiful, customizable loading screen that creates the perfect first impression for players joining your server.
                    </p>
                    <ul class="features-list">
                        <li>Modern Responsive Design</li>
                        <li>Customizable Branding</li>
                        <li>Progress Indicators</li>
                        <li>Music & Sound Support</li>
                    </ul>
                    <button class="cta-button">View Details</button>
                </a>
            </div>
        </section>
        
        <section class="stats">
            <h2 class="section-title">Why Choose Txgls Scripts?</h2>
            <div class="stats-grid">
                <div class="stat-item fade-in">
                    <span class="stat-number">100%</span>
                    <span class="stat-label">QBCore Compatible</span>
                </div>
                <div class="stat-item fade-in">
                    <span class="stat-number">24/7</span>
                    <span class="stat-label">Support Available</span>
                </div>
                <div class="stat-item fade-in">
                    <span class="stat-number">∞</span>
                    <span class="stat-label">Customization Options</span>
                </div>
            </div>
        </section>
        
        <footer class="footer">
            <p class="footer-text">
                Created by <span class="footer-highlight">Txgls</span> | 
                For <span class="footer-highlight">FiveM QBCore</span> | 
                Premium Script Collection
            </p>
        </footer>
    </div>

    <script>
        // Fade in animation on scroll
        const observerOptions = {
            threshold: 0.1,
            rootMargin: '0px 0px -50px 0px'
        };

        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('visible');
                }
            });
        }, observerOptions);

        // Observe all fade-in elements
        document.querySelectorAll('.fade-in').forEach(el => {
            observer.observe(el);
        });

        // Animated counter for stats
        function animateCounter(element, target) {
            let count = 0;
            const increment = target / 100;
            const timer = setInterval(() => {
                count += increment;
                if (count >= target) {
                    element.textContent = target === Infinity ? '∞' : Math.floor(target) + (target === 100 ? '%' : target === 24 ? '/7' : '');
                    clearInterval(timer);
                } else {
                    element.textContent = Math.floor(count);
                }
            }, 20);
        }

        // Trigger counter animation when stats come into view
        const statsObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const number = entry.target.querySelector('.stat-number');
                    const text = number.textContent;
                    if (text === '100%') animateCounter(number, 100);
                    else if (text === '24/7') animateCounter(number, 24);
                    else if (text === '∞') number.textContent = '∞';
                }
            });
        }, { threshold: 0.5 });

        document.querySelectorAll('.stat-item').forEach(item => {
            statsObserver.observe(item);
        });

        // Script detail functions (you can customize these to show more details or redirect)
        function showButcherDetails() {
            // You can replace this with actual links to your script pages
            alert('ButcherJob Script Details\n\nFeatures:\n• Realistic meat processing\n• Multiple processing stages\n• QBCore integration\n• Configurable settings\n\nContact for purchase or demo!');
        }

        function showLoadscreenDetails() {
            // You can replace this with actual links to your script pages
            alert('LoadScreen Script Details\n\nFeatures:\n• Modern responsive design\n• Custom branding options\n• Loading progress indicators\n• Background music support\n\nContact for purchase or demo!');
        }

        // Enhanced hover effects
        document.querySelectorAll('.script-card').forEach(card => {
            card.addEventListener('mouseenter', () => {
                card.style.transform = 'translateY(-15px) scale(1.02)';
            });
            
            card.addEventListener('mouseleave', () => {
                card.style.transform = 'translateY(0) scale(1)';
            });
        });
    </script>
</body>
</html>
